...
describe('envSchema', () => {
  ...
  it('requires TikTok credentials to be set as a pair', () => {
    expect(envSchema.safeParse({ ...validInput, TIKTOK_CLIENT_KEY: 'key-x' }).success).toBe(false);
    expect(envSchema.safeParse({ ...validInput, TIKTOK_CLIENT_SECRET: 'sec-x' }).success).toBe(
      false,
    );
    expect(
      envSchema.safeParse({
        ...validInput,
        TIKTOK_CLIENT_KEY: 'key-x',
        TIKTOK_CLIENT_SECRET: 'sec-x',
      }).success,
    ).toBe(true);
  });
