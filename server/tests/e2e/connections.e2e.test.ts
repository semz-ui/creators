...
describe('Connections flow (e2e)', () => {
  ...
  function startAndComplete(platform: string) {
    return request(app)
      ...
      .then((start) =>
        request(app)
          .get(`${BASE}/callback`)
          .query({ state: stateFromUrl(start.body.authorizationUrl), code: 'auth-code' }),
      );
  }
  ...
  it('links an account through the OAuth round-trip', async () => {
    ...
    expect(start.body.authorizationUrl).toContain('state=');

    const callback = await request(app)
      .get(`${BASE}/callback`)
      .query({ state: stateFromUrl(start.body.authorizationUrl), code: 'auth-code' });
    expect(callback.status).toBe(200);
    expect(callback.body).toMatchObject({ platform: 'facebook', status: 'active' });
    expect(callback.body).not.toHaveProperty('accessToken');

    const list = await request(app).get(BASE).set(auth.headers);
    expect(list.body.items).toHaveLength(1);
    expect(list.body.items[0]).not.toHaveProperty('accessToken');
  });
  ...
  it('reuses (reconnects) rather than duplicating the same platform', async () => {
    ...
    const youtube = first.body.items.find((c: { platform: string }) => c.platform === 'youtube');

    await startAndComplete('youtube');
    const second = await request(app).get(BASE).set(auth.headers);
    const youtubeAgain = second.body.items.filter(
  ...
  it('disconnects a connection', async () => {
    ...
    const id = callback.body.id as string;

    expect((await request(app).delete(`${BASE}/${id}`).set(auth.headers)).status).toBe(204);

    const list = await request(app).get(BASE).set(auth.headers);
    expect(list.body.items.some((c: { id: string }) => c.id === id)).toBe(false);
  });
});
