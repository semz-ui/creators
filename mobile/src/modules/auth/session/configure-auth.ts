...
import { useSessionStore, waitForSessionHydration } from './session.store';
...
/**
 * Restore the session on boot: wait for the persisted state to hydrate from
 * secure storage, then if a refresh token exists mint a fresh access token and
 * load the current user. Resolves to whether a session was restored.
 */
